...
for moduleName in ${MODULE_NAMES}; do
  if ${changedModules[${moduleName}]}; then
    ...
    CMD="aws s3 cp s3://${S3_BUCKET_NAME}/${ARTIFACT_OBJECT_KEY} s3://${S3_BUCKET_NAME}/${APP_STAGE}_module_pipelines/module_source/${moduleName}.zip"
    echo Executing ${CMD}
    ${CMD}
    pipelineExecutionIds[${moduleName}]=$(aws codepipeline start-pipeline-execution --name ${moduleName}_${APP_STAGE}_pipeline --query "pipelineExecutionId" --output text)
...
checkExecutions() {
  ...
  for moduleName in ${MODULE_NAMES}; do
    if [[ ${changedModules[${moduleName}]} = true ]]; then
      status=$(aws codepipeline get-pipeline-execution --pipeline-name ${moduleName}_${APP_STAGE}_pipeline --pipeline-execution-id ${pipelineExecutionIds[${moduleName}]}  --query pipelineExecution.status --output text)
      ...
      if [[ "$status" = "Failed" ]]; then
        anyFailed=true
        echo "${moduleName} failed. Check https://${AWS_DEFAULT_REGION}.console.aws.amazon.com/codesuite/codepipeline/pipelines/${moduleName}_${APP_STAGE}_pipeline/executions/${pipelineExecutionIds[${moduleName}]}/timeline?region=${AWS_DEFAULT_REGION}"
      fi
    fi
  done
}
